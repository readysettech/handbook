...
1. If someone makes a good suggestion invite them to add it to the handbook. Send the person the URL of the relevant page and section and offer to do it for them if they can't. Having them make and send the suggestion will make the change and will reflect their knowledge.
...
Team members with `merge rights`, are team members who have the GitHub permissions to _merge_ merge requests (AKA: maintainer-level permissions).
It is important that team members with maintainer permission use this responsibility responsibly.
Below are a few guidelines for maintainers:
